...
import { applyFailedPaths, Classification, toSettled, toSettledFailure } from './classify'
...
export function settleRawResult<Data>(rawResult: ClassifiedRawResponse<Data>): SettledResponse<Data> {
  ...
  if (classification.outcome === 'failure') return toSettledFailure(classification, status)
